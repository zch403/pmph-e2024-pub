...
let lss_redOp (pred2: i32 -> i32 -> bool)
  ...
  let new_lis = if x_lss == x_len && segments_connect then x_lss+y_lis else if x_len == 0 then y_lis else x_lis
  let new_lcs = if y_lss == y_len && segments_connect then x_lcs+y_lss else if y_len == 0 then x_lcs else y_lcs
